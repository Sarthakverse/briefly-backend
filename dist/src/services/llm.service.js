...
exports.SYSTEM_PROMPT_JSON = void 0;
exports.processTranscript = processTranscript;
const openai_1 = __importDefault(require("openai"));
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const llm_1 = require("../config/llm");
exports.SYSTEM_PROMPT_JSON = `ROLE: Expert meeting analyst/technical writer. Summarize transcripts for mixed audiences (leadership, technical, IC).
...
\`\`\`
`;
async function processManual(meetingId) {
    const responsePath = path_1.default.join(__dirname, '../../llm-responses/default-response.json');
    if (!fs_1.default.existsSync(responsePath)) {
        throw new Error(`Default response file not found at ${responsePath}. Please add your pre-generated LLM output.`);
    }
    return fs_1.default.readFileSync(responsePath, 'utf-8');
}
async function processWithAPI(transcript) {
    const apiKey = llm_1.llmConfig.provider === 'deepseek' ? llm_1.llmConfig.deepseekApiKey : llm_1.llmConfig.openaiApiKey;
    if (!apiKey)
        throw new Error(`API key not set for provider ${llm_1.llmConfig.provider}`);
    const baseURL = llm_1.llmConfig.provider === 'deepseek' ? 'https://api.deepseek.com' : 'https://api.openai.com/v1';
    const openai = new openai_1.default({ apiKey, baseURL });
    const response = await openai.chat.completions.create({
        model: llm_1.llmConfig.provider === 'deepseek' ? 'deepseek-reasoner' : 'gpt-4o-mini',
        messages: [
            { role: 'system', content: exports.SYSTEM_PROMPT_JSON },
...
async function processTranscript(meetingId, transcript) {
    switch (llm_1.llmConfig.provider) {
        case 'manual':
        ...
        default:
            throw new Error(`Unknown LLM provider: ${llm_1.llmConfig.provider}`);
    }
}
