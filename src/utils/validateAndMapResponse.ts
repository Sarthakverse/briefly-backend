...
export function validateAndMapResponse(jsonString: string): ParsedMeetingOutput {
  let data: any;
  try {
  ...
  const hasTechContent =
    (typeof data.detailedSummary.objective === 'string' && data.detailedSummary.objective.trim().length > 0) ||
